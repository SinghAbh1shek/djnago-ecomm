            ...
            for index,row in df.iterrows():
                ...
                if row['Variation type'] in ['Parent only','Parent with variant/s']:
                    parent_product = Product.objects.create(
                        ...
                        product_descriptions = row['Product description'],
                ...
                if row['Variation type'] == 'Variant':
                    parent_product = Product.objects.get(product_sku = row['Parent SKU'])
                    variant_options = []
...
def getProductFromImageName(imageName):
    ...
    try:
        ...
        print(imageName)
        return True , Product.objects.get(product_sku = imageName), imageName
    except Exception as e:
        print(e)
...
def generateOrderPdf(instance, data):
    ...
    config = pdfkit.configuration(wthtmltopdf = path_wthtmltopdf)
